...
- Optionally runs PostProcessing.yaml per-trial to produce visitSummary
- Reads visitSummary from the trial's postproc child run (or falls back to trial run)
...
- Saves detailed failure rows (CSV) with pipetask return codes and log file paths
- Can echo/tail stdout/stderr of each pipetask for quick debugging
"""
...
def maybe_run_postproc(ctx: Context, out_coll: str, trial_dir: Path, visits: List[int]) -> Optional[str]:
    ...
    try:
        ...
        log_failure(ctx, f"{out_coll}-postproc", e, cmd, stdout_log, stderr_log)
        print(f"\n--- postproc FAILED (rc={e.returncode}) tail ---")
...
def run_trial(ctx: Context, params: Dict[str, Any], tag: str, trial_index: int) -> Tuple[str, float, Dict[str, Any]]:
    ...
    # Output collection is a CHAINED collection we keep appending runs to.
    ...
    # Optionally run postproc to create visitSummary; prefer reading from that child collection.
    post_coll = None
    if ctx.run_postproc:
