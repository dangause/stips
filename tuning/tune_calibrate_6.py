...
"""
Nickel calibrateImage tuner (Optuna)
- Runs calibrateImage per-visit with per-trial overrides
- Uses initial_pvi / initial_stars_detector products explicitly
...
- Optional: auto-discover visits if --visits omitted
- Optional: print/tail pipetask logs on failure or always (--echo-logs)
"""
...
# ---------------------------------------
# Discovery helpers (postISR & visits)
# ---------------------------------------
...
def discover_visits(repo: Path, collections: List[str]) -> List[int]:
    """Return all science visits present in given collections."""
    where = "exposure.observation_type='science' AND instrument='Nickel'"
    cmd = ["butler", "query-dimension-records", str(repo), "visit",
           "--collections", ",".join(collections), "--where", where, "--fmt", "json"]
    cp = run(cmd, check=False)
    if cp.returncode != 0:
        return []
    try:
        recs = json.loads(cp.stdout)
...
def write_overrides(ctx: Context, params: Dict[str, Any], tag: str) -> Path:
    ...
    and explicitly point to initial_pvi / initial_stars_detector datasets.
    """
...
def setDefaults(config):
    # Explicitly use these pre-products
    config.initial_pvi = "initial_pvi"
    config.initial_stars_detector = "initial_stars_detector"
...
def run_trial(ctx: Context, params: Dict[str, Any], tag: str, trial_index: int) -> Tuple[str, float, Dict[str, Any]]:
    ...
    for v in ctx.visits:
        ...
        try:
            ...
            if ctx.echo_logs:
                print(f"\n--- pipetask calibrateImage v{v} (rc=0) tail ---")
...
def make_objective(ctx: Context):
    def objective(trial: optuna.Trial) -> float:
        ...
        try:
            ...
            # log a runs row with fail status
            runs_csv = ctx.workdir / "tuning_runs.csv"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tune Nickel calibrateImage with failure-penalized scoring (Optuna)")
    ...
    p.add_argument("--visits", nargs="*", type=int, default=None, help="visit IDs to process (omit to auto-discover)")
    ...
    p.add_argument("--echo-logs", action="store_true", help="Tail pipetask logs even on success")
    p.add_argument("--tail", type=int, default=120, help="How many lines to show when tailing logs")
    return p.parse_args()
...
def main() -> None:
    ...
    if args.visits is None or len(args.visits) == 0:
        # auto-discover all science visits present in postISR + calib/current
        visits_all = discover_visits(repo, [inputs_postisr, "Nickel/calib/current"])
        visits = [v for v in visits_all if v not in set(args.bad)]
    else:
