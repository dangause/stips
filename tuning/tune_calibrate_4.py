...
# CURRENTLY BEST WORKING VERSION SO FAR
...
from pathlib import Path
def _as_path(x):
    # accept Context, str, or Path
    if hasattr(x, "workdir"):
        return Path(x.workdir)
    return Path(x)
...
# --- WRITE OVERRIDES ---
def write_overrides(workdir, params, tag):
    """
    Accepts Context | str | Path. Writes calib_overrides_{tag}.py
    """
    out_dir = _as_path(workdir)
    out_dir.mkdir(parents=True, exist_ok=True)

    L = []

    # ---------------- PSF detection (loosen) ----------------
    thr  = float(params["psf_det.threshold"])
    incM = float(params["psf_det.incMult"])
    L += [
        f"config.psf_detection.thresholdValue = {thr:.3f}",
        f"config.psf_detection.includeThresholdMultiplier = {incM:.3f}",
    ]

    # ---------------- Ensure PSF-pass schema has CircularApertureFlux(12) ----------------
    # This directly addresses the NotFoundError: base_CircularApertureFlux_12_0_instFlux
    L += [
        "cfg = config.psf_source_measurement.plugins['base_CircularApertureFlux']",
        "cfg.doMeasure = True",
        "cfg.radii = [12.0]",
        # keep star-pass apertures simple + present
        "cfg2 = config.star_measurement.plugins['base_CircularApertureFlux']",
        "cfg2.doMeasure = True",
        "cfg2.radii = [8.0, 12.0]",
    ]
    ...
    L += [
        "sel = config.psf_measure_psf.starSelector['objectSize']",
        "sel.doSignalToNoiseLimit = True",
        f"sel.signalToNoiseMin = {float(params['psfsel.snmin']):.3f}",
        "sel.doFluxLimit = False",
        "sel.widthMin = 0.8",
        "sel.widthMax = 8.0",
        f"sel.widthStdAllowed = {float(params['psfsel.widthStdMax']):.3f}",
        "sel.nSigmaClip = 3.0",
        # no reservation in star-poor fields
        "config.psf_measure_psf.reserve.fraction = 0.0",
    ]

    # ---------------- Astrometry matcher (MatchPessimisticB) ----------------
    L += [
        "m = config.astrometry.matcher",
        f"m.maxOffsetPix = {int(params['match.maxOffsetPix'])}",
        f"m.maxRotationDeg = {float(params['match.maxRotationDeg']):.3f}",
        f"m.matcherIterations = {int(params['match.matcherIterations'])}",
        f"m.minMatchDistPixels = {float(params['match.minMatchDistPixels']):.3f}",
        f"m.minMatchedPairs = {int(params['match.minMatchedPairs'])}",
        f"m.minFracMatchedPairs = {float(params['match.minFracMatchedPairs']):.4f}",
        f"m.numBrightStars = {int(params['match.numBrightStars'])}",
        f"m.maxRefObjects = {int(params['match.maxRefObjects'])}",
        f"m.numPatternConsensus = {int(params['match.numPatternConsensus'])}",
        # mild science S/N cut
        "ss = config.astrometry.sourceSelector['science']",
        "ss.doSignalToNoise = True",
        f"ss.signalToNoise.minimum = {float(params['astro_src.snmin']):.3f}",
    ]

    # ---------------- ApCorr selector + robust fit ----------------
    L += [
        "c = config.measure_aperture_correction",
        "c.sourceSelector.name = 'science'",
        "css = c.sourceSelector['science']",
        "css.doSignalToNoise = True",
        f"css.signalToNoise.minimum = {float(params['apcorr.snmin']):.3f}",
        "css.signalToNoise.maximum = None",
        f"c.numSigmaClip = {float(params['apcorr.sigclip']):.3f}",
        f"c.numIter = {int(params['apcorr.niter'])}",
    ]
    ...
    L += [
        ...
        f"nss.signalToNoise.minimum = {float(params['ncf.snmin']):.3f}",
        "nss.doUnresolved = False",
        "nss.doIsolated = False",
    ]

    # ---------------- Persist matches for QA ----------------
    L += [
        "config.optional_outputs = tuple(sorted(set(getattr(config, 'optional_outputs', ())) | "
        "{'photometry_matches','astrometry_matches'}))"
    ]

    over_py = out_dir / f"calib_overrides_{tag}.py"
    over_py.write_text("\n".join(L) + "\n")
    return over_py
...
def sample_params(trial):
    """
    Hyperparams focused on:
      - psf_detection loosened (thresholdValue, includeThresholdMultiplier)
      - psf starSelector['objectSize'] S/N and spread tolerance
      - astrometry.matcher (MatchPessimisticB) tolerances and counts
      - mild astrometry source S/N cut
      - apcorr selectors + robust fit settings
      - PSF normalized calib-flux selector (ncf)

    All other values are held to your requested base values in write_overrides.
    """
    p = {}

    # --- PSF-pass detection ---
    p["psf_det.threshold"] = trial.suggest_float("psf_det.threshold", 2.5, 4.0)          # around 3.0
    p["psf_det.incMult"]   = trial.suggest_float("psf_det.incMult",   4.0, 8.0)          # around 6.0

    # --- PSF star selector (objectSize) ---
    p["psfsel.snmin"]       = trial.suggest_float("psfsel.snmin",     8.0, 20.0)         # around 10
    p["psfsel.widthStdMax"] = trial.suggest_float("psfsel.widthStdMax", 0.30, 0.60)      # around 0.40

    # --- Astrometry matcher (MatchPessimisticB) ---
    p["match.maxOffsetPix"]         = trial.suggest_int("match.maxOffsetPix", 100, 400)  # base ~250
    p["match.maxRotationDeg"]       = trial.suggest_float("match.maxRotationDeg", 0.5, 3.0)    # base ~2.0
    p["match.matcherIterations"]    = trial.suggest_int("match.matcherIterations", 8, 12)      # base 10
    p["match.minMatchDistPixels"]   = trial.suggest_float("match.minMatchDistPixels", 1.0, 2.5)  # tighten endgame
    p["match.minMatchedPairs"]      = trial.suggest_int("match.minMatchedPairs", 8, 20)        # 8..20
    p["match.minFracMatchedPairs"]  = trial.suggest_float("match.minFracMatchedPairs", 0.02, 0.08)  # 0.02..0.08
    p["match.numBrightStars"]       = trial.suggest_int("match.numBrightStars", 150, 300)
    p["match.maxRefObjects"]        = trial.suggest_int("match.maxRefObjects", 4000, 8000)
    p["match.numPatternConsensus"]  = trial.suggest_int("match.numPatternConsensus", 2, 3)

    # --- Astrometry source S/N cut ---
    p["astro_src.snmin"] = trial.suggest_float("astro_src.snmin", 8.0, 20.0)

    # --- ApCorr selector + robust fit ---
    p["apcorr.snmin"]     = trial.suggest_float("apcorr.snmin", 20.0, 40.0)   # base ~30
    p["apcorr.sigclip"]   = trial.suggest_float("apcorr.sigclip", 3.0, 6.0)   # base 5.0
    p["apcorr.niter"]     = trial.suggest_int("apcorr.niter", 3, 5)           # base 4

    # --- PSF normalized calibration flux (ncf) selector ---
    p["ncf.snmin"] = trial.suggest_float("ncf.snmin", 15.0, 35.0)             # base ~25
...
def run_trial(ctx: Context, params: dict, tag: str) -> Tuple[str, float, dict]:
    ...
    overrides = write_overrides(ctx.workdir, params, tag)
