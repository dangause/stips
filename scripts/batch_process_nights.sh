#!/usr/bin/env bash
# batch_process_nights.sh — Process multiple nights through calibs → science → coadds
#
# Usage:
...
if [[ "$SKIP_DOWNLOAD" == "true" && "$SKIP_CALIBS" == "true" && "$SKIP_SCIENCE" == "true" ]]; then
  echo "ERROR: Cannot skip download, calibs, and science (nothing to do)"
  exit 2
fi
...
for i in "${!NIGHTS_ARRAY[@]}"; do
  ...
  if [[ "$SKIP_DOWNLOAD" == "false" ]]; then
    ...
    PYTHON_CMD="$OBS_NICKEL/scripts/fetch_archive_night.py"
    if [[ -n "${LICK_ARCHIVE_DIR:-}" && -f "${LICK_ARCHIVE_DIR}/.venv/bin/python" ]]; then
      PYTHON_CMD="${LICK_ARCHIVE_DIR}/.venv/bin/python $OBS_NICKEL/scripts/fetch_archive_night.py"
    fi
  ...
  if [[ "$SKIP_CALIBS" == "false" && "$NIGHT_SUCCESS" == "true" ]]; then
    ...
    if run_or_dry "$OBS_NICKEL/scripts/10_calibs.sh" --night "$night" 2>&1 | tee -a "$BATCH_LOG"; then
  ...
  if [[ "$SKIP_SCIENCE" == "false" && "$NIGHT_SUCCESS" == "true" ]]; then
    ...
    if run_or_dry "$OBS_NICKEL/scripts/20_science.sh" "${SCIENCE_ARGS[@]}" 2>&1 | tee -a "$BATCH_LOG"; then
...
if [[ "$BUILD_TEMPLATE" == "true" && ${#SUCCESSFUL_NIGHTS[@]} -gt 0 ]]; then
  ...
  if run_or_dry "$OBS_NICKEL/scripts/30_coadds.sh" "${TEMPLATE_ARGS[@]}" 2>&1 | tee -a "$BATCH_LOG"; then
