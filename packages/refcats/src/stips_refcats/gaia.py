...
#: Sync TAP is capped at 2000 rows server-side.
_SYNC_ROW_CAP = 2000


def _sync_adql(adql: str) -> str:
    """Rewrite an async cone ADQL for the row-capped synchronous endpoint.

    Sync TAP returns results inline but caps rows server-side, so take the
    BRIGHTEST rows rather than an arbitrary 2000: ~500 sources per CTIO FOV, so
    this is ample for astrometric/photometric calibration. Pure (no astroquery
    import) so it stays cheaply testable.
    """
    return (
        adql.replace("SELECT ", f"SELECT TOP {_SYNC_ROW_CAP} ", 1)
...
def _launch_gaia_job_sync(adql: str):
    ...
    results inline over HTTP, bypassing that volume. Isolated so tests can mock it.
    """
    from astroquery.gaia import Gaia

    Gaia.MAIN_GAIA_TABLE = _GAIA_TABLE
    Gaia.TIMEOUT = 600
    return Gaia.launch_job(query=_sync_adql(adql), output_format="votable")
